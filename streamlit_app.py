...
# ===== НАСТРОЙКА СТРАНИЦЫ =====
st.set_page_config(
    page_title="HTML Конструктор PRO — Визуальный редактор",
    page_icon="🚀",
    layout="wide",
    initial_sidebar_state="expanded"
)

# ===== РАСШИРЕННЫЙ CSS С ВИЗУАЛЬНЫМИ ПОДСКАЗКАМИ =====
st.markdown("""
<style>
    /* Основные стили */
    .main-header {
    ...
    .block-preview-mini table {
        font-size: 0.6rem;
        border-collapse: collapse;
        width: 100%;
    }
    .block-preview-mini td, .block-preview-mini th {
        border: 1px solid #dee2e6;
        padding: 2px 4px;
    }
    .block-preview-mini .card-preview {
        display: flex;
        gap: 8px;
        align-items: center;
    }
    .block-preview-mini .card-preview .img-placeholder {
        width: 40px;
        height: 40px;
        background: #e9ecef;
        border-radius: 4px;
    }
    .block-preview-mini .card-preview .info {
        font-size: 0.6rem;
    }
    ...
    @media (prefers-color-scheme: dark) {
        ...
        .block-preview-mini { background: #1a1a2e; border-color: #3d3d5d; }
        .block-preview-mini::after { background: linear-gradient(transparent, #1a1a2e); }
        .block-meta .category-badge { background: #3d3d5d; color: #e0e0e0; }
...
if 'block_explanations' not in st.session_state:
    st.session_state.block_explanations = {}
...
def parse_blocks(html):
    ...
    patterns = [
        {
            ...
            'icon': '📊',
            'category': 'Таблицы',
            'explanation': 'Используется для отображения структурированных данных, например, список товаров с ценами, артикулами и статусами.'
        },
        {
            'name': 'Карточка',
            ...
            'icon': '🃏',
            'category': 'Карточки',
            'explanation': 'Используется в каталогах для отображения товаров или услуг. Обычно включает картинку, название, описание и кнопку действия.'
        },
        {
            'name': 'Секция',
            ...
            'icon': '📐',
            'category': 'Секции',
            'explanation': 'Логическая часть страницы, например, "О нас", "Контакты", "Почему мы". Используется для структурирования контента.'
        },
        {
            'name': 'Product-блок',
            ...
            'icon': '🛍️',
            'category': 'Товары',
            'explanation': 'Специализированный блок для интернет-магазинов. Содержит все ключевые элементы товара для быстрой покупки.'
        },
        {
            'name': 'Элемент списка',
            'pattern': r'<li[^>]*>[\s\S]*?<\/li>',
            ...
            'icon': '📋',
            'category': 'Списки',
            'explanation': 'Одиночный пункт в упорядоченном или неупорядоченном списке. Применяется для меню, характеристик, шагов инструкции.'
        },
        {
            'name': 'Кнопка',
            ...
            'icon': '🔘',
            'category': 'Интерактив',
            'explanation': 'Элемент управления, по которому пользователь кликает для выполнения действия. Важен для форм и навигации.'
        },
        {
            ...
            'icon': '🖼️',
            'category': 'Медиа',
            'explanation': 'Визуальный элемент, показывающий картинку. Используется для иллюстрации товаров, баннеров, аватарок.'
        },
        {
            'name': 'Форма',
            ...
            'icon': '📝',
            'category': 'Формы',
            'explanation': 'Позволяет пользователю отправлять данные: заявки, обратная связь, регистрация. Содержит поля и кнопку отправки.'
        },
        {
            'name': 'Навигация',
            ...
            'icon': '🧭',
            'category': 'Навигация',
            'explanation': 'Меню сайта, помогающее пользователю перемещаться между страницами. Обычно содержит ссылки на главную, каталог, контакты.'
        },
        {
            'name': 'Footer',
            ...
            'icon': '📌',
            'category': 'Структура',
            'explanation': 'Нижняя часть страницы с контактами, ссылками на соцсети, информацией о компании и копирайтом.'
        },
    ]
    ...
    for i, m in enumerate(unique):
        blocks.append({
            ...
            'is_improved': False,
            'preview_html': truncate_preview(m['content'])
        })
    ...
    if not blocks:
        blocks.append({
            ...
            'icon': '📄',
            'category': 'Документ',
            'explanation': 'Весь HTML-файл целиком. Используется, если не удалось разбить на отдельные блоки.',
            'size': len(html),
            'lines': html.count('\n') + 1,
            'tags': [],
            'is_improved': False,
            'preview_html': html[:500] + ('...' if len(html) > 500 else '')
        })
    
    return blocks

def truncate_preview(html):
    """Создаёт визуальное мини-превью блока (упрощённый HTML)"""
    # Удаляем скрипты и стили
    clean = re.sub(r'<script[^>]*>.*?</script>', '', html, flags=re.DOTALL)
    clean = re.sub(r'<style[^>]*>.*?</style>', '', clean, flags=re.DOTALL)
    # Обрезаем до 300 символов, но сохраняем структуру
    if len(clean) > 300:
        clean = clean[:300] + '...'
    return clean
...
def show_guide():
    st.markdown("""
    <div class="guide-box">
        <h3 style="margin-bottom: 12px;">🎓 Как пользоваться HTML Конструктором PRO</h3>
        <div class="guide-step">
            <div class="num">1</div>
            <div class="text"><strong>Загрузите ваш HTML-файл</strong> — нажмите <kbd>Browse files</kbd> в боковой панели и выберите файл.</div>
        </div>
        <div class="guide-step">
            <div class="num">2</div>
            <div class="text"><strong>Выберите блок</strong> для работы — кликните на любой блок в списке слева. Вы увидите его визуальное превью и описание.</div>
        </div>
        <div class="guide-step">
            <div class="num">3</div>
            <div class="text"><strong>Улучшите блок</strong> — нажмите <kbd>🤖 Улучшить ИИ</kbd> (встроенный ИИ) или отправьте блок в ChatGPT для более продвинутой обработки.</div>
        </div>
        <div class="guide-step">
            <div class="num">4</div>
            <div class="text"><strong>Обновите блок</strong> — после улучшения нажмите <kbd>🔄 Обновить</kbd>, чтобы сохранить изменения в проекте.</div>
        </div>
        <div class="guide-step">
            <div class="num">5</div>
            <div class="text"><strong>Пересоберите HTML</strong> — нажмите <kbd>🚀 Пересобрать и скачать</kbd>, чтобы получить готовый HTML-файл со всеми улучшениями.</div>
        </div>
        <div style="margin-top: 12px; background: #dbeafe; padding: 10px 16px; border-radius: 8px;">
            💡 <strong>Подсказка:</strong> Если вы новичок, просто следуйте шагам по порядку. Все кнопки подписаны понятно.
        </div>
    </div>
    """, unsafe_allow_html=True)
...
st.markdown('<p class="sub-header">Визуальный редактор блоков HTML с ИИ-улучшением, аналитикой и генератором</p>', unsafe_allow_html=True)
...
if not st.session_state.blocks:
            ...
            with st.expander(f"Блок #{i+1}: {gen['description'][:50]}..."):
                ...
                if st.button(f"Использовать блок #{i+1}", key=f"use_gen_{i}"):
                    new_block = {
                        ...
                        'is_improved': False,
                        'preview_html': truncate_preview(gen['html'])
                    }
    ...
    # ===== ПОКАЗЫВАЕМ ГАЙД ПРИ ПЕРВОМ ЗАПУСКЕ (можно скрыть) =====
    ...
    # === КОЛОНКА 1: СПИСОК БЛОКОВ С ПРЕВЬЮ ===
    with col1:
        st.markdown("### 📦 Блоки")
        ...
        for i, block in enumerate(filtered_blocks):
            ...
            category_badge = f'<span class="badge-category">{block.get("category", "Другое")}</span>'
            
            # Визуальное превью (мини-виджет)
            preview_content = block.get('preview_html', '')
            preview_html = f'<div class="block-preview-mini">{preview_content}</div>'
            ...
            <div class="{card_class}">
                ...
                <div class="block-desc">{block.get('description', '')}</div>
                <div class="block-meta">
                    {category_badge}
                    <span>📏 {block.get('lines', 0)} строк</span>
                    <span>🏷️ {len(block.get('tags', []))} тегов</span>
                </div>
                {preview_html}
                <div style="margin-top: 4px; font-size: 0.6rem; color: #6c757d;">⬆️ Наведите для предпросмотра (упрощённо)</div>
            </div>
            """
            ...
            with col_action1:
                if st.button(f"🤖 Улучшить", key=f"quick_improve_{i}", use_container_width=True):
                    ...
                    with st.spinner("🔄 ИИ улучшает блок..."):
                        ...
                        block['preview_html'] = truncate_preview(improved)
    ...
    # === КОЛОНКА 2: РЕДАКТОР С ОБЪЯСНЕНИЕМ ===
    with col2:
        ...
        if st.session_state.selected_index >= 0 and st.session_state.selected_index < len(st.session_state.blocks):
            ...
            # Показываем объяснение блока
            st.info(f"📖 **Что это за блок?** {block.get('explanation', 'Описание отсутствует')}")
            ...
            with col_buttons1:
                if st.button("🔄 Обновить", use_container_width=True):
                    ...
                    block['is_improved'] = True
                    block['preview_html'] = truncate_preview(new_content)
            ...
            with col_buttons2:
                if st.button("↩️ Отменить", use_container_width=True):
                    if st.session_state.undo_history:
                        ...
                        st.session_state.blocks[last['index']]['preview_html'] = truncate_preview(last['content'])
                        st.session_state.analysis_data = analyze_blocks(st.session_state.blocks)
            ...
            with col_buttons4:
                if st.button("🎨 Применить стили", use_container_width=True):
                    ...
                    block['is_improved'] = True
                    block['preview_html'] = truncate_preview(styled)
                    st.session_state.analysis_data = analyze_blocks(st.session_state.blocks)
    ...
    # === КОЛОНКА 3: ПРЕВЬЮ В КОНТЕКСТЕ ===
    with col3:
        st.markdown("### 🖼️ Превью в реальном времени")
        if st.session_state.selected_index >= 0 and st.session_state.selected_index < len(st.session_state.blocks):
            block = st.session_state.blocks[st.session_state.selected_index]
            ...
            2. **Выберите блок** из списка слева — вы увидите его визуальное превью
            3. **Улучшите блок** кнопкой "🤖 Улучшить ИИ"
            4. **Обновите блок** и **пересоберите HTML**
            5. **Скачайте** готовый файл
            """)
            st.markdown("""
            <div class="info-box">
                <strong>💡 Подсказка:</strong> Каждый блок имеет своё описание. Наводите на блоки, чтобы понять их назначение.
            </div>
            """, unsafe_allow_html=True)
...
<div style="text-align:center;color:#6c757d;font-size:0.8rem;padding:1rem 0;">
    🚀 HTML Конструктор PRO • Визуальный редактор для новичков и профи<br>
    <span style="font-size:0.7rem;">Поддерживает: встроенный ИИ, OpenAI, DeepSeek, Claude • Дашборд • Генератор • Превью блоков</span>
</div>
""", unsafe_allow_html=True)
